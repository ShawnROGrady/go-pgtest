...
const updateTaskArchive = `-- name: UpdateTaskArchive :one
UPDATE
	tasks
SET
	...
	priority = (CASE WHEN $6::bool
			THEN $7::task_priority
			ELSE priority::task_priority
		END),

	-- this doesn't work since on the application side '@status' is just a
	-- string, and even if we don't pass '@set_status' the server will
	-- return an error if that string isn't a valid 'task_status'. So if
	-- '@set_status' is false, then '@status' is expected to be the empty
	-- string '' which is not a valid task_status.
	status = (CASE WHEN $8::bool
			THEN $9
			ELSE status
		END),

	assignee_id = (CASE WHEN $10::bool
			THEN $11
			ELSE assignee_id
		END),
...
type UpdateTaskArchiveParams struct {
	ID             int32
	SetName        bool
	Name           string
	SetDescription bool
...
func (q *Queries) UpdateTaskArchive(ctx context.Context, arg UpdateTaskArchiveParams) (Task, error) {
	row := q.db.QueryRow(ctx, updateTaskArchive,
		arg.ID,
		arg.SetName,
		arg.Name,
		arg.SetDescription,
